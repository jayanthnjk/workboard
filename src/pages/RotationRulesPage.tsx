  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Rotation Rules</h1>
        <button onClick={() => { setEditingRule(null); resetForm(); setIsModalOpen(true) }} className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90">
          Add Rule
        </button>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingRule ? 'Edit Rule' : 'Add Rule'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <FormField label="Name" required>
            <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600" required />
          </FormField>
          <FormField label="Type" required>
            <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value as RotationRuleType })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600">
              {ruleTypes.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </FormField>
          <FormField label="Description">
            <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600" rows={2} />
          </FormField>
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Min Rest Hours" required>
              <input type="number" value={formData.minRestHours} onChange={e => setFormData({ ...formData, minRestHours: parseInt(e.target.value) })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600" min={0} required />
            </FormField>
            <FormField label="Max Consecutive Days" required>
              <input type="number" value={formData.maxConsecutiveDays} onChange={e => setFormData({ ...formData, maxConsecutiveDays: parseInt(e.target.value) })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600" min={1} required />
            </FormField>
          </div>
          <FormField label="Priority" required>
            <input type="number" value={formData.priority} onChange={e => setFormData({ ...formData, priority: parseInt(e.target.value) })} className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600" min={1} required />
          </FormField>
